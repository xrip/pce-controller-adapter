...
static inline void set_ws2812b_HSV(uint8_t i, uint8_t _hue, uint8_t sat, uint8_t _bri)
{
    ...
    // старт 
    dma_channel_configure (led_dma_chan, &dma_ch0, &ws2812_pio->txf[ws2812_sm], led_buffer, LED_COUNT, true);
}

//-------------------------------------------------------------------------------
static inline void init_ws2812b()
{
    uint offset = pio_add_program(ws2812_pio, &ws2812c_program);
    ws2812c_program_init(ws2812_pio, ws2812_sm, offset, WS2812_PIN);
	...
	dma_ch0 = dma_channel_get_default_config(led_dma_chan);
